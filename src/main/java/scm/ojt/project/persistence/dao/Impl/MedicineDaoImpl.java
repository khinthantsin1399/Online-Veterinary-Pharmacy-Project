...
public class MedicineDaoImpl implements MedicineDao {
    ...
    public static String SELECT_MEDICINE_LIST_HQL = "FROM Medicine m "
            + "WHERE m.unit_in_stock > 0 AND m.deletedAt IS NULL ";

    SimpleDateFormat formatter = new SimpleDateFormat("dd/MM/yyyy");

    public static String SELECT_MEDICINE_BY_SEARCH_DATA = "AND (m.medicine_name like :medicine_name ) ";

    public static String SELECT_MEDICINE_BY_CODE_HQL = "SELECT m FROM Medicine m "
            + "WHERE m.medicine_code = :medicine_code ";

    public static String SELECT_MEDICINE_BY_NAME_HQL = "SELECT m FROM Medicine m "
            + "WHERE m.medicine_name = :medicine_name ";
     ...
     * <p>
     * method to check whether medicine name already exists or not
     * </p>
     * 
     * @param medName
     * @return
    ...
    public Medicine isMedicineNameExist(String medName) {
        String HqlQuery = "SELECT m FROM Medicine m where m.medicine_name = :medicine_name";
        Query queryisExist = this.sessionFactory.getCurrentSession().createQuery(HqlQuery);
        queryisExist.setParameter("medicine_name", medName);
        Medicine resultMedicine = (Medicine) queryisExist.uniqueResult();
        return resultMedicine;
    }
     ...
     * <p>
     * method to check whether update medicine code already exists or not
     * </p>
     * 
     * @param medicineCode
     * @return
    ...
    public List<Medicine> dbUpdatedMedicineExistList(String medicineCode) {
        Query queryMedicine = this.sessionFactory.getCurrentSession().createQuery(SELECT_MEDICINE_BY_CODE_HQL);
        queryMedicine.setParameter("medicine_code", medicineCode);
        List<Medicine> medicineList = (List<Medicine>) queryMedicine.list();
        return medicineList;
    }
     ...
     * <p>
     * method to check whether update medicine name already exists or not
     * </p>
     * 
     * @param medicineName
     * @return
    ...
    public List<Medicine> dbUpdatedMedicineNameExistList(String medicineName) {
        Query queryMedicine = this.sessionFactory.getCurrentSession().createQuery(SELECT_MEDICINE_BY_NAME_HQL);
        queryMedicine.setParameter("medicine_name", medicineName);
        List<Medicine> medicineList = (List<Medicine>) queryMedicine.list();
        return medicineList;
    }
}